        ...
        <div className="flex flex-col lg:flex-row gap-4 md:gap-6 items-start mt-4 md:mt-5 w-full min-w-0">
                ...
                {(() => {
                  ...
                  if (submittedWinners.length > 0) {
                    ...
                    effectiveStandings = [...submittedWinners]
                      .sort((a, b) => a.position - b.position)
                      .map((w) => {
                        const reg = registrants.find(
                          (r) => r.inGameId === w.inGameId,
                        );
                        return {
                          position: w.position,
                          inGameId: w.inGameId,
                    ...
                    const gfMatch = gfRound?.matches?.find(
                    ...
                    if (bracketChampion) {
                      ...
                      const sfLosers = (
                        wbRoundsData[wbRoundsData.length - 1]?.matches ?? []
                      )
                    ...
                    if (effectiveStandings.length === 0) {
                      effectiveStandings = (tournamentResults ?? []).map(
                        (e, idx) => ({
                          position: Number(
                            e.position ?? e.final_placement ?? idx + 1,
                          ),
                          inGameId: String(e.in_game_id ?? e.inGameId ?? ""),
                          displayName: String(
                            e.username ?? e.in_game_id ?? e.inGameId ?? "—",
                          ),
                    ...
                    <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                      ...
                      <div className="grid grid-cols-3 gap-1.5 sm:gap-3">
                        {[
                          ...
                          const entry = effectiveStandings.find(
                            (e) => e.position === pos,
                          );
                ...
                <>
                  ...
                  <div className="hidden md:block overflow-x-auto">
                    <table className="w-full min-w-160">
                      <thead>
                        <tr className="border-b border-slate-800/60 bg-slate-950/20">
                          {["Player", "Status", "Registered", "Actions"].map(
                            (h) => (
                              <th
                                key={h}
                  ...
                  {registrantsTotalPages > 1 && (
                    <div className="flex items-center justify-between gap-3 px-5 py-3 border-t border-slate-800/60 bg-slate-950/10">
                      <p className="text-[11px] text-slate-500">
                        {(registrantsPage - 1) * REGISTRANTS_PAGE_SIZE + 1}–
                        {Math.min(
                          registrantsPage * REGISTRANTS_PAGE_SIZE,
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() =>
                            setRegistrantsPage((p) => Math.max(1, p - 1))
                          }
                          ...
                          .map((item, idx) =>
                            item === "…" ? (
                              ...
                              <button
                                key={item}
                                onClick={() =>
                                  setRegistrantsPage(item as number)
                                }
          ...
          <div className="w-full lg:w-72 shrink-0 space-y-4 lg:sticky lg:top-6">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
              ...
              >
                {[
                  ...
                  {
                    label: "Type",
                    value: (tournament.tournamentType ?? "—").replace(
                      /_/g,
                      " ",
                    ),
                  },
                  ...
                  {isLoadingCoOrgs ? (
                    ...
                    <div className="space-y-1.5">
                      {coOrganizers.map((co, idx) => {
                        ...
                        const coUserId =
                          u?._id ??
                          (typeof co.user_id === "string" ? co.user_id : "");
                        const coName = u
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                void handleRemoveCoOrg(String(coUserId), coName)
                              }
                              title="Remove"
            ...
            {(() => {
                ...
                return (
                  <div className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
                    ...
                    <div className="p-4 space-y-2">
                      {RESULT_SLOTS.map(
                        ({ pos, icon, label, nameClass, bg }) => {
                          const winner = top3.find((w) => w.position === pos);
                          if (!winner) return null;
                          const reg = registrants.find(
                            (r) => r.inGameId === winner.inGameId,
                          );
                          const displayName =
                            reg?.displayName ?? winner.inGameId;
                          return (
                            <div
                              key={label}
              ...
              const champion = gfMatch?.participants?.find(
                (p) => p.result === "win",
              );
              if (!champion) return null;
              const runnerUp = gfMatch?.participants?.find(
                (p) => p.result === "loss",
              );
              const wbRoundsData = bracketRounds.filter(
                (r) => r !== gfRound && r.bracket !== "grand_final",
              ...
              const sfLosers =
                sfRound?.matches
                  ...
                  ?.flatMap(
                    (m) =>
                      m.participants?.filter((p) => p.result === "loss") ?? [],
                  ) ?? [];
              ...
              return (
                <div className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
                  ...
                  <div className="p-4 space-y-2">
                    {bracketPlacements.map(
                      ({ icon, label, player, nameClass, bg }) => (
                        <div
                          key={label}
                          className={`flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl border ${bg}`}
                        >
                          ...
                          <span
                            className={`text-sm truncate text-right ${nameClass}`}
                          >
