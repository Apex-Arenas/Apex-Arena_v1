...
import {
  TrendingUp, ChevronDown, RefreshCw, Phone, X,
  Loader2, CheckCircle2, Clock, AlertTriangle, Banknote, RotateCcw,
} from "lucide-react";
...
const STATUS_META: Record<EarningStatus, { label: string; cls: string; dot: string }> = {
...
function EarningCard({ earning, onClaim }: { earning: Earning; onClaim: () => void }) {
  const statusMeta = STATUS_META[earning.status];
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {earning.status === "pending_claim" && (
          <button onClick={onClaim}
            ...
            <Banknote className="w-3.5 h-3.5" /> Claim Earnings
          </button>
        )}
        {earning.status === "paid" && (
...
export default function EarningsPage() {
  ...
  const [claiming, setClaiming]  = useState<Earning | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const list = (Array.isArray(res.data) ? res.data : (d.earnings ?? d.data ?? [])) as Earning[];
        setEarnings(list);
      }
    } catch {
  ...
  const unclaimed  = earnings.filter(e => e.status === "pending_claim").length;
  const pending    = earnings.filter(e => e.status === "claimed" || e.status === "processing").length;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative bg-slate-900 border-b border-slate-800/60 overflow-hidden">
        <div className="absolute -top-40 right-0 w-[700px] h-[400px] rounded-full bg-orange-500/6 blur-3xl pointer-events-none" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-8 pt-10 pb-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="font-display text-4xl sm:text-5xl font-bold text-white leading-none">Earnings</h1>
              <p className="text-base text-slate-400 mt-3">Your organizer earnings from entry fees and prize pool refunds.</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {earnings.map((e) => (
              <EarningCard key={e._id} earning={e} onClaim={() => setClaiming(e)} />
            ))}
          </div>
        )}
      </div>
