...
import {
  RotateCcw, ChevronDown, RefreshCw, Phone, X,
...
const STATUS_META: Record<RefundStatus, { label: string; cls: string; dot: string }> = {
...
function ClaimModal({ refund, onClose, onClaimed }: { refund: Refund; onClose: () => void; onClaimed: () => void }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5 hover:border-slate-700 transition-colors">
      ...
      <div className="rounded-xl bg-slate-800/60 border border-slate-700/50 px-3 py-2.5 mb-4">
        ...
        {refund.reason_note && (
          <p className="text-xs text-slate-500 mt-0.5 italic">{refund.reason_note}</p>
        )}
      </div>
...
export default function RefundsPage() {
  ...
  const [claiming, setClaiming] = useState<Refund | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const list = (Array.isArray(res.data) ? res.data : (d.refunds ?? d.data ?? [])) as Refund[];
        setRefunds(list);
      }
    } catch {
  ...
  const unclaimed = refunds.filter(r => r.status === "pending_claim").length;
  const pending   = refunds.filter(r => r.status === "claimed" || r.status === "processing").length;
  const totalPaid = refunds.filter(r => r.status === "paid").reduce((s, r) => s + r.amount, 0);

  const statItems = [
    { icon: RotateCcw,    iconColor: "text-cyan-400",   bg: "from-cyan-500/25 to-indigo-500/20",   label: "Total",     value: loading ? "—" : String(refunds.length) },
    { icon: AlertTriangle,iconColor: "text-amber-400",  bg: "from-amber-500/25 to-orange-500/20",  label: "Unclaimed", value: loading ? "—" : String(unclaimed)       },
    { icon: Clock,        iconColor: "text-indigo-400", bg: "from-indigo-500/25 to-violet-500/20", label: "Pending",   value: loading ? "—" : String(pending)         },
    { icon: CheckCircle2, iconColor: "text-green-400",  bg: "from-green-500/25 to-teal-500/20",    label: "Refunded",  value: loading ? "—" : fmtGhs(totalPaid)       },
  ];
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative bg-slate-900 border-b border-slate-800/60 overflow-hidden">
        <div className="absolute -top-40 right-0 w-[600px] h-[400px] rounded-full bg-cyan-500/5 blur-3xl pointer-events-none" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-8 pt-10 pb-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="font-display text-4xl sm:text-5xl font-bold text-white leading-none">Refunds</h1>
              <p className="text-base text-slate-400 mt-3">Entry fee refunds owed to you — claim via Mobile Money.</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {refunds.map((r) => (
              <RefundCard key={r._id} refund={r} onClaim={() => setClaiming(r)} />
            ))}
          </div>
        )}
      </div>
