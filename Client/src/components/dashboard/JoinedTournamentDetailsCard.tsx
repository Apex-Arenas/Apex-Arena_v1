...
export default function JoinedTournamentDetailsCard({ reg }: Props) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-900/65 backdrop-blur-sm hover:border-slate-700/90 hover:-translate-y-0.5 hover:shadow-[0_24px_44px_-30px_rgba(14,165,233,0.55)] transition-[border-color,transform,box-shadow] duration-300"
    >
      ...
      <div className="relative h-36 sm:aspect-16/10 sm:h-auto overflow-hidden shrink-0">
        ...
        {hasImage ? (
          <>
            ...
            <div className="absolute inset-0 bg-linear-to-br from-orange-500/18 via-transparent to-cyan-500/22" />
          </>
        ) : (
          <>
            <div className="absolute inset-0 bg-linear-to-br from-orange-950 via-slate-900 to-cyan-950" />
            <div className="absolute inset-0 flex items-center justify-center">
              <Gamepad2 className="w-14 h-14 text-slate-700/90" />
            </div>
          </>
        )}

        {/* Bottom fade — only covers the bottom third for chip readability */}
        <div className="absolute inset-0 bg-linear-to-t from-slate-900/95 via-slate-900/25 to-transparent" />

        {/* Top-left game tag */}
        {reg.gameName && (
          <div className="absolute top-2.5 left-2.5">
            <span className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-slate-950/80 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-300 backdrop-blur-sm">
        ...
        {tourMeta && (
          <div className="absolute top-2.5 right-2.5">
            <span
              className={`flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] px-2.5 py-1 rounded-full bg-slate-950/80 backdrop-blur-sm border border-white/10 ${tourMeta.text}`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${tourMeta.dot}`}
              />
              {tourMeta.label}
            </span>
          </div>
        ...
        {prizeGhs ? (
          <div className="absolute bottom-2.5 right-2.5">
            <span className="text-[11px] font-semibold text-amber-300 bg-slate-950/85 backdrop-blur-sm px-2 py-0.5 rounded-full border border-amber-400/20">
              {prizeGhs}
            </span>
          </div>
        ) : reg.finalPlacement ? (
          <div className="absolute bottom-2.5 right-2.5">
            <span className="text-[11px] font-semibold text-orange-300 bg-slate-950/85 backdrop-blur-sm px-2 py-0.5 rounded-full border border-orange-400/20">
              #{reg.finalPlacement}
            </span>
          </div>
        ) : null}
      ...
      <div className="relative px-4 pt-3 pb-4 flex flex-col gap-2.5">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <h4 className="font-display text-[15px] font-bold text-white leading-tight truncate group-hover:text-cyan-100 transition-colors">
              {reg.tournamentTitle}
            </h4>
            <p className="text-[11px] text-slate-500 mt-1 truncate">
              {reg.gameName ?? "Unknown Game"} ·{" "}
              {reg.registrationType === "team" ? "Team" : "Solo"}
            </p>
          </div>
          <span
            className={`shrink-0 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] px-2.5 py-1 rounded-full bg-slate-800/90 border border-white/5 ${regMeta.text}`}
          >
        ...
        {/* Detail grid */}
        <div className="grid grid-cols-2 gap-x-3 gap-y-3 border-t border-slate-800/80 pt-2.5">
          <div>
          ...
          <div>
            ...
            <p
              className="text-[11px] font-semibold text-orange-300 truncate"
