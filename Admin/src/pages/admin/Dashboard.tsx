...
function StatCard({
  ...
  return (
    <div
      className={`relative rounded-2xl border ${c.border} bg-slate-900/70 p-5 overflow-hidden group hover:border-opacity-60 transition-[transform,border-color,box-shadow] duration-300 ease-out hover:-translate-y-0.5 hover:shadow-[0_22px_40px_-30px_rgba(14,165,233,0.7)]`}
    >
      <div
        className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${c.glow} shadow-[inset_0_0_40px_0px]`}
      />
      <div className="relative">
        <div className="flex items-start justify-between mb-3">
          ...
          <div
            className={`w-8 h-8 rounded-xl ${c.bg} flex items-center justify-center shrink-0 transition-transform duration-300 ease-out group-hover:scale-105`}
          >
            <Icon className={`w-4 h-4 ${c.text}`} />
          </div>
        </div>
        <p
          className={`font-bold text-white ${large ? "text-4xl" : "text-2xl"}`}
        >
          {value}
        </p>
...
function QuickLink({
  ...
  return (
    <Link
      to={to}
      className={`flex items-center gap-3 p-4 rounded-xl border ${c.border} bg-slate-900/60 hover:bg-slate-800/60 transition-[background-color,border-color,transform,box-shadow] duration-250 ease-out group hover:-translate-y-0.5 hover:shadow-[0_16px_32px_-24px_rgba(148,163,184,0.8)]`}
    >
      <div
        className={`w-9 h-9 rounded-xl ${c.bg} flex items-center justify-center shrink-0 transition-transform duration-250 ease-out group-hover:scale-105`}
      >
      ...
      <ChevronRight className="w-4 h-4 text-slate-600 group-hover:text-slate-400 transition-[color,transform] duration-250 ease-out group-hover:translate-x-0.5 shrink-0" />
    </Link>
  );
}
  ...
  return (
    <div className={`grid ${cols} gap-4`}>
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 animate-pulse space-y-3"
        >
...
export default function AdminDashboard() {
  ...
  const fullName = [admin?.firstName, admin?.lastName]
    ...
    .join(" ")
    .trim();
  const name = fullName || admin?.username || "Admin";
        ...
        <div className="relative p-5 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            ...
            <button
              onClick={fetchStats}
              disabled={loading}
              className="self-start sm:self-auto flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-700 bg-slate-900/70 text-sm text-slate-300 hover:text-white hover:border-slate-600 transition-colors disabled:opacity-50"
            >
              <RefreshCw
          ...
          <section>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard
                ...
                icon={UserX}
                accent="red"
                subtext="Accounts blocked by admin"
              />
              <StatCard
                label="Unverified"
                ...
                icon={AlertTriangle}
                accent="amber"
                subtext="Email not yet confirmed"
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <StatCard
                ...
                icon={TrendingUp}
                accent="cyan"
                subtext="Registrations in last 24 h"
              />
              <StatCard
                label="New This Week"
                ...
                icon={TrendingUp}
                accent="blue"
                subtext="Registrations this week"
              />
              <StatCard
                ...
                icon={TrendingUp}
                accent="purple"
                subtext="Registrations this month"
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <StatCard
                ...
                icon={ShieldAlert}
                accent="red"
                subtext="Failed sign-in attempts"
              />
              <StatCard
                ...
                icon={Lock}
                accent="orange"
                subtext="Accounts locked after failures"
              />
              <StatCard
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <QuickLink
                ...
                icon={BadgeCheck}
                label="Verifications"
                sub="Review organizer applications"
                accent="amber"
              />
              <QuickLink
                to="/admin/payouts"
                icon={Wallet}
                label="Payouts"
                sub="Approve withdrawal requests"
                accent="emerald"
              />
              <QuickLink
                to="/admin/games"
